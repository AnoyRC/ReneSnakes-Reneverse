...
public class Snake : MonoBehaviour {
    ...
    public ColorTemplate[] colorTemplates = new ColorTemplate[7];
    ...
    void GetARandomTemplate()
    {
        if (isPlayer)
        {
            List<ColorTemplate> selected =  new List<ColorTemplate>();
            foreach(Asset asset in ReneverseManager.NFTCounter) {
                if (asset.AssetName == "Normal")
                {
                    selected.Add(colorTemplates[0]);
                }
                if (asset.AssetName == "Yellow") 
                {
                    selected.Add(colorTemplates[1]);
                }
                if (asset.AssetName == "Red")
                {
                    selected.Add(colorTemplates[2]);
                }
                if (asset.AssetName == "Black")
                {
                    selected.Add(colorTemplates[3]);
                }
                if (asset.AssetName == "Maroon")
                {
                    selected.Add(colorTemplates[4]);
                }
                if (asset.AssetName == "Blue")
                {
                    selected.Add(colorTemplates[5]);
                }
                if (asset.AssetName == "Green")
                {
                    selected.Add(colorTemplates[6]);
                }
            }
            colorTemplate = selected[Random.Range(0, selected.Count)];
        }
        if(!isPlayer)
        {
